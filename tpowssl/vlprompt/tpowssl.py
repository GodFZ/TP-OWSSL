...
from tpowssl.vlprompt.prompted_transformers import PromptedTransformer
from tpowssl.vlprompt.clip_local import ModifiedResNet, VisionTransformer, CLIP
...
class TPOWSSL(CLIP):
    ...
    def __init__(
        ...
        self.model_name = "tpowssl_" + clip_name[5:]
    ...
    def encode_text(self, class_names: List[str]) -> torch.Tensor:
        ...
        tokenized_text = clip.tokenize(class_names).cuda(non_blocking=True) # torch.Size([102, 77])
        eot_tokens = tokenized_text.argmax(dim=-1) # torch.Size([102])

        with torch.no_grad():
            token_embeddings = self.token_embedding(tokenized_text)  # torch.Size([102, 77, 512])

        prefix = token_embeddings[:, :1, :]                          # torch.Size([102, 1, 512])
        suffix = token_embeddings[:, 1 : -self.n_token_context, :]   # torch.Size([102, 72, 512])
        ...
        if self.learn_global_prompt or self.n_global_prompts > 1:
            ...
            #在text_features上用global_prompt替换一部分
        else:
            text_features = self._default_encode_text(class_names)

        if self.learn_local_prompts or self.n_local_prompts > 1:
            local_text_features = self._most_efficient_encode_text(prefix, self.local_prompts, suffix, eot_tokens)
            #在local_text_features上用local_prompts替换一部分
        else:
            local_text_features = text_features

        # print(global_prompt.shape) # torch.Size([4, 4, 512])
        # print(self.local_prompts.shape) # torch.Size([4, 4, 512])
    ...
    def forward(
        ...
        if text_features is None:
            ...
            # print(text_features.shape)       # torch.Size([102, 4, 512])
            # print(local_text_features.shape) # torch.Size([102, 4, 512])
        ...
        # print(image_features.shape) # torch.Size([32, 512])
        # print(local_features.shape) # torch.Size([32, 196, 512])
    ...
    def initialize_prompt(self) -> NoneType:
        ...
        # print(embedding.shape) # torch.Size([1, 77, 512])
        global_prompt_init = embedding[:, 1 : 1 + self.template_init_tokens, :]
        # print(global_prompt_init.shape) # torch.Size([1, 4, 512])
        ...
        # print(self.global_prompt.shape) # torch.Size([4, 4, 512])
        # print(self.local_prompts.shape) # torch.Size([4, 4, 512])
