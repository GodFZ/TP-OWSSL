...
from simgcd.config import car_root
...
class CarsDataset(Dataset):
    ...
    def __init__(self, train=True, limit=0, data_dir=car_root, transform=None):
        ...
        metas = os.path.join(data_dir, 'cars_train_annos.mat') if train else os.path.join(data_dir, 'cars_test_annos_withlabels.mat')
        # data_dir = os.path.join(data_dir, 'cars_train/') if train else os.path.join(data_dir, 'cars_test/')
        data_dir = os.path.join(data_dir, 'car_ims/') if train else os.path.join(data_dir, 'cars_test/')
        # data_dir = 'G:/code/SimGCD-main/SimGCD-main/datasets/standford cars/car_ims/car_ims'
        ...
        for idx, img_ in enumerate(labels_meta['annotations'][0]):
            ...
            # self.data.append(data_dir + str(img_[5][0]))
            self.data.append(data_dir + str(img_[0][0]))
