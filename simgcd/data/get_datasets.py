from simgcd.data.data_utils import MergedDataset

from simgcd.data.cifar import get_cifar_10_datasets, get_cifar_100_datasets
from simgcd.data.herbarium_19 import get_herbarium_datasets
from simgcd.data.stanford_cars import get_scars_datasets
from simgcd.data.imagenet import get_imagenet_100_datasets, get_imagenet_1k_datasets
from simgcd.data.cub import get_cub_datasets
from simgcd.data.fgvc_aircraft import get_aircraft_datasets
from simgcd.data.flower import get_flower_datasets
from simgcd.data.pets import get_pets_datasets

from copy import deepcopy
import pickle
import os

from simgcd.config import osr_split_dir
...
def target_transform(x, target_transform_dict):
...
def get_datasets(dataset_name, train_transform, test_transform, args):
    ...
    # target_transform = target_transform(target_transform_dict)

    # 使用 functools.partial 来"预填充"函数参数
    from functools import partial
    target_transform_partial = partial(target_transform, target_transform_dict=target_transform_dict)

    for dataset_name, dataset in datasets.items():
        if dataset is not None:
            dataset.target_transform = target_transform_partial
